...
$(function () {
    function login(e) {
        ...
        ajaxJSON.post('/login/authenticate', user, true,
            function (res) {
                $('div.error').text('')
                sessionStorage.setItem('user', JSON.stringify(res));
                sessionStorage.setItem('accessToken', res.Token);
                debugger
                if (res.StudentUserName) {
                    window.open("student/list-subject.html", "_self");
                }
                else if (res.ManagerUserName) {
                    window.open("manager/exam.html", "_self");
                }
            })
            .fail(function (res) {
                $('div.error').text('Tên truy cập hoặc mật khẩu không chính xác')
            });
    }
    $('#login').click(login);
    $('#password').keydown((e) => {
